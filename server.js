...
// Update the generateCSV function to handle star ratings and include location
async function generateCSV(csvPath) {
  try {
    ...
    const formattedData = responses.map((response) => {
      const baseData = {
        Name: response.name || "NA", // Use the name field from response
    ...
    const fields = ["Name", "Survey Title", "Department", "Tenure", "Submission Date", "Submission Time"]
...
app.post("/api/responses", async (req, res) => {
  try {
    ...
    const { surveyId, userId, department, tenure, answers, name } = req.body

    // Create response object
    const response = new Response({
      surveyId,
      userId: userId || "anonymous_user", // Keep userId for tracking
      name: name || "NA", // Store the name separately
      location: "Not specified", // Set default location since it's no longer collected
      department,
      answers,
      tenure,
    })
