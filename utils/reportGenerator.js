...
class ReportGenerator {
  ...
  // New approach: Analyze each survey individually
  async generateAnalysis() {
    try {
      const responses = await this.readCSV()

      // Group responses by survey title
      const surveyGroups = {}
      ...
      // Create department summary across all surveys
  ...
  analyzeSurvey(surveyTitle, responses) {
    ...
    // Extract questions from the first response (all responses in same survey have same questions)
    ...
    // Calculate satisfaction for this survey
  ...
  // Process checkbox questions separately to handle multiple selections
  ...
  // Categorize responses into satisfaction levels
  categorizeSatisfactionResponse(answer) {
    const answerLower = answer.toString().toLowerCase().trim()

    // Star ratings - handle both "X stars" and just "X"
  ...
  createOverallSummary(doc, analysis) {
    ...
    // Statistics - properly aligned to left
  ...
  createSurveyAnalysisPage(doc, surveyData) {
    ...
    // Department breakdown for this survey
  ...
  determineQuestionType(answer, allAnswers = []) {
    ...
    // Checkbox detection (contains comma)
    if (answer.includes(",")) {
      return "Checkbox"
    }
